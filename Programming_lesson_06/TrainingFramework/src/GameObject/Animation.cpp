...
void Animation::CaculateWorldMatrix()
{
	Matrix m_Sc, m_T;
...
Animation::Animation (int m_numFrames, float m_frameTime, std::shared_ptr<Models> model, std::shared_ptr<Shaders> shader, std::shared_ptr<Texture> texture)
{
	m_pModel = model;
	m_pShader = shader;
	m_pCamera = nullptr;
...
Animation::Animation(std::shared_ptr<Models> model, std::shared_ptr<Shaders> shader, Vector4 color)
	: Sprite2D()
{
	m_pModel = model;
	m_pShader = shader;
...
void Animation::Draw()
{
	...
	iTempShaderVaribleGLID = -1;
	iTempShaderVaribleGLID = m_pShader->GetUniformLocation((char*)"u_");
	if (iTempShaderVaribleGLID != -1)
...
void Animation::SetText(std::string text)
{
	m_Text = text;
}

std::string Animation::GetText()
{
	return m_Text;
}

void Animation::Set2DPosition(GLfloat width, GLfloat height)
{
	m_Vec2DPos.x = width;
	m_Vec2DPos.y = height;
...
void Animation::Set2DPosition(Vector2 pos)
{
	m_Vec2DPos = pos;
...
Vector2 Animation::Get2DPosition()
{
	return m_Vec2DPos;
}


void Animation::SetSize(GLint width, GLint height)
{
	m_iWidth = width;
	m_iHeight = height;
	m_Vec3Scale = Vector3((float)m_iWidth / screenWidth, (float)m_iHeight / screenHeight, 1);
